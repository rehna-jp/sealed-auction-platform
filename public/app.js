...
function setupSocketListeners() {
  ...
  socket.on("auctionCreated", (auction) => {
    ...
    addNotification(`New auction: ${auction.title}`, "auction", {
  ...
  socket.on("auctionClosed", (auction) => {
    ...
    if (auction.winner) {
      addNotification(
        `Auction "${auction.title}" closed! Winner: ${auction.winner}`,
        "success",
        { auctionId: auction.id },
      );
    } else {
      addNotification(
        `Auction "${auction.title}" closed without bids`,
        "info",
        { auctionId: auction.id },
      );
    }
  ...
  socket.on("bidPlaced", (data) => {
    ...
    addNotification("New bid placed on auction!", "bid", {
...
function showLoadingIndicator() {
  ...
  if (!loader) {
    ...
    loader.className =
      "fixed bottom-4 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white px-6 py-3 rounded-full shadow-lg animate-pulse z-50";
    loader.innerHTML = `
...
function addAuctionToGrid(auction, prepend = true) {
  ...
  auctionsGrid.insertAdjacentHTML(
...
function createAuctionCard(auction) {
  ...
  const statusClass =
    auction.status === "closed" || isExpired ? "bg-red-500" : "bg-green-500";
  const statusText =
    auction.status === "closed" ? "Closed" : isExpired ? "Expired" : "Active";

  return `
        <div class="auction-card glass-effect rounded-xl p-6 hover:shadow-lg transition-all duration-300 animate-fade-in" data-auction-id="${
          auction.id
        }">
            ...
            <div class="space-y-2 mb-4">
                ...
                <div class="flex justify-between text-sm">
                    <span>Bids:</span>
                    <span class="font-semibold" data-bid-count="${
                      auction.id
                    }">${auction.bidCount}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span>Ends:</span>
            ...
            <div class="flex space-x-2">
                ...
                <button onclick="viewAuctionDetails('${
                  auction.id
    ...
    auction.startingBid
                ...
                <button onclick="openARPreview(${
                  auction.id
                })" class="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 rounded-lg text-sm transition-colors" title="AR Preview">
...
function updateAuctionInGrid(auction) {
  const auctionCard = document.querySelector(
...
function updateBidCount(auctionId, bidCount) {
  const bidCountElement = document.querySelector(
    `[data-bid-count="${auctionId}"]`,
  );
  if (bidCountElement) {
...
function openARPreview(auctionId) {
  const auction = (window._allAuctions || auctions).find(
...
function viewAuctionDetails(auctionId) {
  // Use the new detailed auction view
...
function addNotification(message, type = "info", data = null) {
  ...
  if (
    ...
    !notificationPreferences.types.results
  )
    return;
...
function getNotificationIcon(type) {
  switch (type) {
    ...
    case "auction":
      return "fa-gavel text-purple-500";
    case "bid":
      return "fa-hand-holding-usd text-green-500";
    default:
...
let savedFilterPresets = JSON.parse(
...
function getFilterState() {
  ...
  const statuses = [...document.querySelectorAll(".filter-status:checked")].map(
    (el) => el.value,
  );
  return {
    categories,
...
function applyFilters() {
  ...
  if (f.categories.length) {
    results = results.filter((a) =>
      f.categories.includes((a.category || "other").toLowerCase()),
    );
  }
  if (f.statuses.length) {
    results = results.filter((a) => {
      const status =
        a.status || (new Date(a.endTime) > new Date() ? "active" : "closed");
  ...
  if (f.dateFrom)
    results = results.filter(
      (a) => new Date(a.endTime) >= new Date(f.dateFrom),
    );
  if (f.dateTo)
    results = results.filter(
      (a) => new Date(a.endTime) <= new Date(f.dateTo + "T23:59:59"),
    );
...
function renderFilterResults(results) {
  ...
  if (!results.length) {
    grid.innerHTML =
      '<p class="text-gray-500 col-span-full">No auctions match your filters.</p>';
    return;
  }
    ...
    .map(
      (a) => `
        <div class="glass-effect rounded-xl p-4 cursor-pointer hover:ring-2 hover:ring-purple-500 transition-all" onclick="switchTab('auctions')">
            ...
            <p class="text-sm text-gray-400 mt-1">Starting: $${
              a.startingBid || 0
            }</p>
            <p class="text-xs text-gray-500 mt-1">Ends: ${
...
function resetFilters() {
  document
    .querySelectorAll(".filter-category, .filter-status")
    .forEach((el) => {
      el.checked = false;
    });
...
function loadFilterPreset(index) {
  ...
  document.querySelectorAll(".filter-category").forEach((el) => {
    el.checked = p.categories.includes(el.value);
  });
  document.querySelectorAll(".filter-status").forEach((el) => {
    el.checked = p.statuses.includes(el.value);
  });
  document.getElementById("filterPriceMin").value = p.priceMin || "";
  document.getElementById("filterPriceMax").value =
    p.priceMax === Infinity ? "" : p.priceMax;
...
function renderFilterPresets() {
  ...
  if (!savedFilterPresets.length) {
    container.innerHTML =
      '<span class="text-gray-500 text-sm">No saved presets yet.</span>';
    return;
  }
  container.innerHTML = savedFilterPresets
    .map(
      (p, i) => `
...
function locateMe() {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      status.textContent = `📍 Location found: ${userLocation.lat.toFixed(
        4,
...
function renderNearbyAuctions() {
  ...
  const withCoords = auctions.map((a, i) => ({
    ...
    lat:
      a.lat ||
      (userLocation
        ? userLocation.lat + (Math.random() - 0.5) * 0.5
        : 51.5 + (Math.random() - 0.5) * 0.5),
    lng:
      a.lng ||
      (userLocation
        ? userLocation.lng + (Math.random() - 0.5) * 0.5
  ...
  if (userLocation && radius > 0) {
    nearby = withCoords.filter(
      (a) =>
        haversineKm(userLocation.lat, userLocation.lng, a.lat, a.lng) <= radius,
    );
  }
  nearby.sort((a, b) => {
      ...
      nearby
        ...
        .map((a) => {
          ...
          return `<div class="flex items-center gap-2 text-sm text-left mb-1">
                ...
                ${
                  dist
                    ? `<span class="text-gray-400 ml-auto">${dist}</span>`
                    : ""
                }
            </div>`;
        })
        .join("") ||
      '<p class="text-gray-400 text-sm">No auctions in range.</p>';
  }

  if (!nearby.length) {
    container.innerHTML =
      '<p class="text-gray-500 text-sm col-span-full">No auctions found in this area.</p>';
    return;
  }
  container.innerHTML = nearby
    ...
    .map((a) => {
      ...
      return `<div class="glass-effect rounded-xl p-4 cursor-pointer hover:ring-2 hover:ring-purple-500 transition-all" onclick="switchTab('auctions')">
            ...
            ${
              dist
                ? `<p class="text-xs text-purple-400 mt-1"><i class="fas fa-map-marker-alt mr-1"></i>${dist}</p>`
                : ""
            }
    ...
    searchInput.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        document.getElementById(
          "mapStatus",
        ).textContent = `Searching for "${searchInput.value}"… (requires geocoding API)`;
      }
    });
  }
});
...
function loadVideoPlaylist() {
  ...
  videoPlaylistData = auctions.slice(0, 12).map((a, i) => ({
    ...
    url: "",
    duration: `${Math.floor(Math.random() * 3) + 1}:${String(
...
function renderVideoPlaylist() {
  ...
  if (!videoPlaylistData.length) {
    container.innerHTML =
      '<p class="text-gray-500 text-sm">No auction previews available.</p>';
    return;
  }
  container.innerHTML = videoPlaylistData
    .map(
      (v, i) => `
        <div class="flex gap-3 p-2 rounded-lg cursor-pointer hover:bg-white/10 transition-colors ${
          currentVideoIndex === i ? "ring-2 ring-purple-500" : ""
        }" onclick="playVideo(${i})">
            <div class="relative flex-shrink-0 w-20 h-12 rounded overflow-hidden bg-gray-800">
                <img src="${v.thumbnail}" alt="${
        v.title
      }" class="w-full h-full object-cover" onerror="this.style.display='none'">
                <span class="absolute bottom-0.5 right-0.5 text-xs bg-black/70 px-1 rounded">${
                  v.duration
                }</span>
            </div>
...
function playVideo(index) {
  ...
  analyticsEl.textContent = `Watching: ${v.title} | Quality: ${
    document.getElementById("videoQualitySelect").value
  }`;
...
const VOICE_COMMANDS = {
  ...
  help: () => {
    switchTab("voice");
    speakFeedback(
      "Available commands: show auctions, create auction, my bids, open filter, open map, open video, open network, scroll down, scroll up, stop listening.",
    );
  },
};
...
function initVoiceRecognition() {
  ...
  if (noteEl)
    noteEl.textContent = "Speech recognition is supported in this browser.";
...
function renderVoiceHistory() {
  ...
  container.innerHTML = voiceHistory
    .map(
      (h) => `
        ...
        <div class="text-gray-400 text-xs mb-1">${h.response}</div>`,
    )
    .join("");
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById("activeAuctions").textContent =
        stats.auctions?.active_auctions || 0;
      document.getElementById("revenue30d").textContent = `$${
        stats.revenue?.revenue_30d || 0
      }`;
      document.getElementById("securityAlerts").textContent =
        stats.security?.openAlerts || 0;
    }
  } catch (error) {
...
function renderUsersTable(users) {
  ...
  if (!users || users.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="6" class="text-center py-4 text-gray-400">No users found</td></tr>';
    return;
  }
    ...
    .map(
      (user) => `
        <tr class="border-b border-white/5">
            ...
            <td class="py-2 text-xs text-gray-400">${new Date(
              user.created_at,
            ...
            <td class="py-2">
                <div class="flex gap-1">
                    <select onchange="updateUserRole('${
                      user.id
                    }', this.value)" class="text-xs bg-white/10 border border-white/20 rounded px-1 py-0.5">
                        <option value="user" ${
                    ...
                    <button onclick="toggleUserStatus('${
                      user.id
                    }')" class="text-xs bg-white/10 border border-white/20 rounded px-2 py-0.5 hover:bg-white/20">
...
function renderUsersPagination(data) {
  ...
  if (data.page > 1) {
    html += `<button onclick="loadUsers(${
      data.page - 1
    })" class="px-2 py-1 text-xs bg-white/10 border border-white/20 rounded hover:bg-white/20">Prev</button>`;
  }

  for (
    let i = Math.max(1, data.page - 2);
    i <= Math.min(data.totalPages, data.page + 2);
    i++
  ) {
    html += `<button onclick="loadUsers(${i})" class="px-2 py-1 text-xs ${
      i === data.page ? "bg-purple-600" : "bg-white/10 border border-white/20"
    } rounded hover:bg-white/20">${i}</button>`;
  }

  if (data.page < data.totalPages) {
    html += `<button onclick="loadUsers(${
      data.page + 1
    })" class="px-2 py-1 text-xs bg-white/10 border border-white/20 rounded hover:bg-white/20">Next</button>`;
  }
...
function openShareModal(auctionId, auctionTitle, startingBid) {
  ...
  if (messageElement)
    messageElement.value = `Check out this auction: "${auctionTitle}" - Starting bid: ${startingBid} XLM`;
...
function addVRControlsToUI() {
  ...
  if (navContainer) {
    ...
    vrButton.className =
      "px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white rounded-lg hover:from-purple-600 hover:to-indigo-700 transition-all duration-200 shadow-lg";
...
function toggleVRPerformance() {
  if (window.vrIntegration) {
    ...
    showNotification(
      `VR performance mode: ${isPerformanceMode ? "Optimized" : "Normal"}`,
      "info",
    );
  }
}
